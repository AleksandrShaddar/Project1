...
users = sqlalchemy.Table(
    ...
    sqlalchemy.Column("adress", sqlalchemy.String(150)),
)
...
class User(BaseModel):
    ...
    surname: str = Field(title="Username", min_length=2)
    birthday: date = Field(title="Birthday")
    email: EmailStr = Field(title="Email", max_length=128)
    adress: str = Field(title="Password", min_length=5)
...
async def create_note(count: int):
    for i in range(1, count + 1):
        query = users.insert().values(
            ...
            birthday=datetime.now(), email=f"mail{i}@mail.ru", adress=f"city{i}")
